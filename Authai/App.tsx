...
import { analyzeContent, mintCertificate, getUserProfile, reportVerification } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  // Helper to sync credits from server
  const syncUserCredits = async (email: string) => {
  ...
  const handleCertification = async () => {
    ...
    if (user) await syncUserCredits(user.email);
    ...
    <div className="relative">
      <PaymentModal
        ...
        onSuccess={async (amount) => {
          if (user) {
            await syncUserCredits(user.email);
          } else {
      ...
      >
        ...
        {currentView === 'profile' && user && <UserProfile email={user.email} credits={credits} onLogout={handleLogout} />}
