...
export const mintCertificate = async (
    ...
    const certPayload = {
        ...
        owner: owner,
        issuedTo: issuedTo || owner, // New field
    ...
    if (supabase) {
        try {
            ...
            const dbPayload = {
                ...
                issued_to: certPayload.issuedTo,
    ...
    try {
        const response = await fetch(`${API_URL}/certificate/mint`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ analysisResult, content, contentType, owner, issuedTo })
        });
...
export const reportVerification = async (reportData: any) => {
    try {
...
export const getUserPaymentHistory = async (email: string) => {
    // Mock for now or fetch from backend if endpoint exists
    return [
        { id: 'PAY-101', date: '2024-02-14', amount: '₹19.00', status: 'SUCCESS' },
        { id: 'PAY-102', date: '2024-01-20', amount: '₹19.00', status: 'SUCCESS' }
    ];
};
