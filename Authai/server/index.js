...
// --- RATE LIMITING ---
const IP_LIMITS = {}; // In-memory store for demo (Use Redis for prod)
const MAX_FREE_SCANS_PER_IP = 10; // Security against automated spam
const MAX_FREE_CERTS_PER_USER = 3;
...
const rateLimitMiddleware = (req, res, next) => {
    ...
    if (!IP_LIMITS[ip]) {
        IP_LIMITS[ip] = { count: 0, resetTime: now + 3600000 }; // 1 hour window
    }

    if (now > IP_LIMITS[ip].resetTime) {
        IP_LIMITS[ip] = { count: 0, resetTime: now + 3600000 };
    }

    if (IP_LIMITS[ip].count >= MAX_FREE_SCANS_PER_IP) {
...
app.post('/api/certificate/mint', async (req, res) => {
    const { analysisResult, content, contentType, owner, issuedTo } = req.body;
    
    // Check Free Limit
    if (owner && owner !== 'ANONYMOUS') {
        const { data: user } = await supabase
            .from('users')
            .select('credits, role') // Assuming we might map 'credits' to 'certificates' or adds a new column
            .eq('email', owner)
            .single();

        // For this implementation, we'll check against existing certificates if we don't have a count column,
        // OR we just use 'credits' as the gatekeeper. 
        // User asked for "3 Free", then pay.
        // Let's implement logic: Check number of certs owned.
        ...
        if (!error && count >= MAX_FREE_CERTS_PER_USER) {
            // Check if they have credits to "pay" for the extra
            // Assuming 1 Credit = 1 Extra Cert
            if (user.credits < 1) {
                return res.status(402).json({ error: "Free limit reached. Please purchase credits." });
            }
            
            // Deduct Credit
             await supabase.from('users').update({ credits: user.credits - 1 }).eq('email', owner);
        }
    }
    ...
    const cert = {
        ...
        owner,
        issued_to: issuedTo || owner, // New field map
...
app.post('/api/report', async (req, res) => {
    const { reportType, description, relatedId, reporter } = req.body;
    
    console.log(`[REPORT] Type: ${reportType}, ID: ${relatedId}, By: ${reporter}`);
    
    // Ideally save to DB 'reports' table
