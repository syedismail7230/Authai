...
const CertificateView: React.FC<CertificateViewProps> = ({ data, onReset }) => {
  ...
  const [issuedToInput, setIssuedToInput] = useState(data.issuedTo || '');

  return (
    <div className="max-w-4xl mx-auto animate-in zoom-in duration-500 pb-12">

      {/* Input for Issued To (Only if not already set/downloading) */}
      {!data.issuedTo && !isDownloading && (
        <div className="mb-8 p-4 bg-neo-yellow border-4 border-black shadow-neo flex flex-col md:flex-row gap-4 items-center">
          <span className="font-bold whitespace-nowrap uppercase">Certificate Issued To:</span>
          <input
            type="text"
            className="flex-1neo-input p-2 border-2 border-black font-mono focus:outline-none"
            placeholder="Enter Name or Organization"
            value={issuedToInput}
            onChange={(e) => {
              setIssuedToInput(e.target.value);
              data.issuedTo = e.target.value; // Mutate local for preview
            }}
          />
        </div>
      )}
      ...
      <div ref={certRef} className="bg-white border-4 md:border-[12px] border-double border-neo-black p-4 md:p-12 shadow-neo relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left max-w-2xl mx-auto mb-12 font-mono text-sm">
            ...
            <div className="border-b-2 border-gray-200 pb-2">
              ...
              <p className="font-bold uppercase">{issuedToInput || data.issuedTo || data.owner}</p>
            </div>
            <div className="border-b-2 border-gray-200 pb-2">
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-white border-4 border-neo-black p-8 shadow-neo">
        ...
        <div className="order-1 md:order-2 flex justify-center items-center bg-gray-50 p-8 border-2 border-dashed border-gray-300">
          ...
          >
            ...
            <div className={`font-black text-xl uppercase z-10 leading-none mb-1 ${data.verdict === 'HUMAN' ? 'text-neo-green' :
