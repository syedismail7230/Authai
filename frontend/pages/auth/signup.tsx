...
export default function Signup() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/auth/send-otp`, {
        email: formData.email,
      });
      setShowOtp(true);
    } catch (err: any) {
      setError(err.response?.data?.message || 'FAILED TO TRANSMIT SIGNAL');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] flex flex-col font-sans text-black selection:bg-black selection:text-white items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white border-4 border-black shadow-[12px_12px_0_0_rgba(0,0,0,1)] p-8">
        ...
        <p className="font-mono text-sm text-center mb-8 uppercase font-bold">&gt;_ NEW_ENTITY_INITIALIZATION</p>
        ...
        <form onSubmit={showOtp ? handleVerifyOtp : handleSendOtp} className="space-y-6">
          {!showOtp && (
            <>
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-widest mb-2 font-mono">SYS_REFERRAL_CODE [OPTIONAL]</label>
                <input
                  type="text"
                  value={formData.referralCode}
                  onChange={(e) => setFormData({ ...formData, referralCode: e.target.value })}
                  placeholder="&gt; INPUT_REFERRAL_NODE"
          ...
          >
            {loading ? 'PROCESSING...' : showOtp ? 'FINALIZE REGISTRATION' : 'REQUEST AUTH PIN'}
          </button>
        </form>
        ...
        <div className="mt-8 text-center border-t-4 border-black pt-4">
          <p className="font-mono text-xs uppercase font-bold">
            EXISTING_ENTITY?{' '}
            <a href="/auth/login" className="underline hover:bg-black hover:text-white p-1 transition-colors">
              EXECUTE_LOGIN
            </a>
          </p>
        </div>
