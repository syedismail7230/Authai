...
import { Lock, User, Key, X, ShieldAlert, UserPlus } from 'lucide-react';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (mode === 'LOGIN') {
                ...
                // data.user has { id, email, role, credits }
                onLogin(data.user.email, data.user.role === 'ADMIN');
                onClose();
            } else {
                const data = await registerUser(email, password);
                setSuccessMsg('Identity Created. Logging in...');
                setTimeout(() => {
                    onLogin(data.user.email, data.user.role === 'ADMIN');
                    onClose();
                }, 1000);
            ...
            <div className="bg-white border-4 border-neo-black shadow-neo w-full max-w-md relative">
                ...
                <div className="bg-neo-black text-white p-6 border-b-4 border-neo-black">
                    ...
                    <p className="font-mono text-xs text-gray-400 mt-1">
                        {mode === 'LOGIN' ? 'IDENTITY VERIFICATION REQUIRED' : 'CREATE FORENSIC PROFILE'}
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    {successMsg && (
                        <div className="bg-neo-green/10 border-l-4 border-neo-green p-3 flex items-start gap-2 text-neo-green text-sm font-bold animate-pulse">
                            <ShieldAlert size={16} className="shrink-0 mt-0.5" />
                            {successMsg}
                        </div>
                    )}
                    ...
                    >
                        {loading ? 'PROCESSING...' : (mode === 'LOGIN' ? 'ESTABLISH SESSION' : 'MINT NEW PROFILE')}
                    </button>
                    ...
                    <div className="text-center">
                        <button
                            type="button"
                            onClick={toggleMode}
                            className="text-sm font-bold underline decoration-2 underline-offset-4 hover:text-neo-blue"
                        >
                            {mode === 'LOGIN' ? 'NO CREDENTIALS? REGISTER PROTOCOL ->' : '<- RETURN TO LOGIN'}
                        </button>
                    </div>
                </form>
